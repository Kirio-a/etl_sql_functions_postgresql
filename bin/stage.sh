...
# проверка выполнения очистки
    RESULT=$(echo "select count(*) from $table_name" | PGPASSWORD=$pgpswd psql -d mydb)
    array=($RESULT)
...
# вызов процесса начала заполнения таблицы etl_run и придание переменной etl_run_id значение id
        ...
        etl_run=${array[2]} 
        echo "etl_run_id:  "$etl_run           
        PGPASSWORD=$pgpswd psql -d mydb -c "\COPY $table_name($fields) FROM '$local_path/etl_files/$csv' WITH (FORMAT CSV, HEADER);" 
# проверка выполнения копирования
        RESULT=$(echo "select count(*) from $table_name" | PGPASSWORD=$pgpswd psql -d mydb)
        array=($RESULT)
        str_cnt=${array[2]}
        RESULT=$(wc -l $local_path"/etl_files/$csv")
        array=($RESULT)
        str_cnt_csv=${array[0]}
