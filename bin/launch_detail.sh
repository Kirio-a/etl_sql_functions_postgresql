...
RESULT=$(echo "select date_part('day', loaded_dttm) = date_part('day', now()) from stage.sql_functions_slava limit 1;" | PGPASSWORD=$pgpswd psql -d mydb)
array=($RESULT)
today=${array[2]}
# провверка по метке "finished" предыдущего процесса
RESULT=$(echo "SELECT detail.check_etl_finish(19, 3)" | PGPASSWORD=$pgpswd psql -d mydb)
array=($RESULT)
...
if [[ $today == 't' && $stage_finished == 't' && $detail_finished_slava != 't' ]]
    ...
    source "$local_path/bin/detail.sh"
    flag=1
fi
...
today=${array[2]}
# провверка по метке "finished" предыдущего процесса
RESULT=$(echo "SELECT detail.check_etl_finish(17, 3)" | PGPASSWORD=$pgpswd psql -d mydb)
array=($RESULT)
stage_finished=${array[2]}
# Проверка по метке "finished" теущего процесса (чтобы не гоняло по кругу)
RESULT=$(echo "SELECT detail.check_etl_finish($etl_id, $step_num)" | PGPASSWORD=$pgpswd psql -d mydb)
...
etl_id=16
# проверка по дате изменений в stage
RESULT=$(echo "select date_part('day', loaded_dttm) = date_part('day', now()) from stage.sql_functions limit 1;" | PGPASSWORD=$pgpswd psql -d mydb)
array=($RESULT)
today=${array[2]}
# провверка по метке "finished" предыдущего процесса
RESULT=$(echo "SELECT detail.check_etl_finish(15, 3)" | PGPASSWORD=$pgpswd psql -d mydb)
array=($RESULT)
stage_finished=${array[2]}
# Проверка по метке "finished" теущего процесса (чтобы не гоняло по кругу)
RESULT=$(echo "SELECT detail.check_etl_finish($etl_id, $step_num)" | PGPASSWORD=$pgpswd psql -d mydb)
